...
subString start end str = take takeval (drop dropval str) where
    dropval = max 0 start
    takeval = max (end - dropval + 1) 0

-- drop 3 "Hello, World!" -> "lo, World!"
-- take (7-3+1) "lo, World!" -> "lo, W"
...
lowerAndGreater n list = go list (0, 0)
  where
    ...
    go sublist (before, after) =
      if null sublist
        then show n ++ " is greater than " ++ show before ++ " elements and lower than " ++ show after ++ " elements"
        else res (tail sublist)
      where
        res t
          | head sublist < n = go t (before + 1, after)
          | head sublist > n = go t (before, after + 1)
          | otherwise = go t (before, after)
